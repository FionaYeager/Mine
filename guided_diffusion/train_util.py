...
class TrainLoop:
    ...
    def run_loop(self):
        ...
        while (
            ...
            i += 1
            ds = MyTrainData("/root/autodl-tmp/lxf/Mine/polyp/brainMRI/test/", train=False, img_scale=1, transform=False)
            ...
            # if self.step  == 28000:
            #     self.save()
            if self.step + self.resume_step > 40000 and (self.step + self.resume_step) % 4000 == 0 :# self.step + self.resume_step > 44000,
                # print("ooo")
                dice = 0
                iou = 0
                for val_step in range(52):#242,220,447,411
    ...
    def valforward(self, img, m):
        ...
        dice = dice_score(pred_start.cpu().numpy(), m.squeeze().cpu().numpy())
        iou = iou_score2(pred_start.cpu().numpy(), m.squeeze().cpu().numpy())
        return dice, iou
