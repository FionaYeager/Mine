...
from .utils import staple, dice_score,dice_coeff2, norm
...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        assert t.shape == (B,)
        # print("x:",x.shape)
        model_output, x5, x4, x3 = model(x, self._scale_timesteps(t), **model_kwargs)
        ...
        if self.model_mean_type == ModelMeanType.PREVIOUS_X:
            ...
            if self.model_mean_type == ModelMeanType.START_X:
                ...
                # print("yes")
                # pred_xstart = th.where(pred_xstart > 0.5, 1, 0)
            else:
                pred_xstart = process_xstart(
                    self._predict_xstart_from_eps(x_t=x, t=t, eps=model_output)
                )
            # betat = (0.0001*(1000-t)+0.02*(t-1))/(1000-1)
            # arfat = 1 - betat
            # a = 1/np.sqrt(arfat.cpu().numpy())
            # b = (1-arfat.cpu().numpy())
            # model_mean = _extract_into_tensor(a, t, x.shape) * x- _extract_into_tensor(b/self.sqrt_one_minus_alphas_cumprod, t, model_output.shape) * model_output
    ...
    def p_sample_loop_known(
        ...
        # noise = th.add(noise,img[:, -1:, ...])
        # mm = img[:, -1:, ...].repeat(1, 4, 1, 1)
        x_noisy = torch.cat((img[:, :-1, ...], noise), dim=1)  # add noise as the last channel
        # x_noisy = torch.add(x_noisy,mm)
        img = img.to(device)
        ...
        # return final["sample"], x_noisy, img, final["cal"], cal_out, final["pred_xstart"]
        # return final["sample"], x_noisy, img, final["model_output"]
        return final["sample"], x_noisy, img, final["pred_xstart"]
    ...
    def ddim_sample_loop_known(
        ...
        t = th.randint(20, 21, (b,), device=device).long().to(device)
        noise = th.randn_like(img[:, :1, ...]).to(device)
    ...
    def training_losses_segmentation(self, model, classifier, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            ...
            model_output, x5, x4, x3 = model(x_t, self._scale_timesteps(t), **model_kwargs)
            ...
            terms["bce"] = 0.7*F.binary_cross_entropy_with_logits(model_output, target.float()) +\
                           0.1*F.binary_cross_entropy_with_logits(x5, target.float()) + \
                           0.1*F.binary_cross_entropy_with_logits(x4, target.float()) + \
                           0.1*F.binary_cross_entropy_with_logits(x3, target.float())
            # terms["bce"] = F.binary_cross_entropy_with_logits(model_output, target.float())
            # lossbound = BoundaryLoss()
            model_output = torch.sigmoid(model_output)
            # terms["boundary"] = Active_contour_loss(target, model_output)
            x5 = torch.sigmoid(x5)
            x4 = torch.sigmoid(x4)
            x3 = torch.sigmoid(x3)
            # terms["smoothl1"] = mean_flat((target - model_output) ** 2).mean()
            smoothl1 = th.nn.SmoothL1Loss(size_average=None, reduce=None, reduction='mean', beta=1.0)
            # terms["loss_cal"] = mean_flat((res - cal) ** 2)
            lossdice = DiceLoss()

            # focal = FocalLoss()
            # terms["smoothl1"] = 0.7 * mean_flat((target - model_output) ** 2).mean() + \
            #                     0.1 * mean_flat((target - x5) ** 2).mean() + \
            #                     0.1 * mean_flat((target - x4) ** 2).mean() + \
            #                     0.1 * mean_flat((target - x3) ** 2).mean()
            # terms["smoothl1"] = smoothl1(model_output, target)
            # terms["train_dice"] = lossdice(model_output, target)
            terms["smoothl1"] = 0.7*smoothl1(model_output, target) + \
                                0.1*smoothl1(x5, target) + \
                                0.1*smoothl1(x4, target) + \
                                0.1*smoothl1(x3, target)
            terms["train_dice"] = 0.7*lossdice(model_output, target) + \
                                  0.1*lossdice(x5, target) + \
                                  0.1*lossdice(x4, target) + \
                                  0.1*lossdice(x3, target)
            # terms["smoothl1"] = smoothl1(model_output, target)
            # terms["train_dice"] = lossdice(model_output, target)

            terms['loss'] = terms["train_dice"] + terms["smoothl1"] + 0.5*terms["bce"]#+ 0.001*terms["mssim"]
    ...
    def training_losses_mine(self, model, classifier, x_start, t, model_kwargs=None, noise=None):
        if model_kwargs is None:
            model_kwargs = {}
        if noise is None:
            noise = th.randn_like(x_start[:, -1:, ...])
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            ...
            model_output, bound = model(x_t, self._scale_timesteps(t), **model_kwargs)
            if self.model_var_type in [
                ModelVarType.LEARNED,
                ModelVarType.LEARNED_RANGE,
            ]:
            ...
            model_output = torch.sigmoid(model_output)
            terms["mse_diff"] = mean_flat((target - model_output) ** 2).mean()
            lossdice = DiceLoss()
            terms["train_dice"] = lossdice(model_output, target)
            lossmssim = MSSSIM()
            terms['mssim'] = lossmssim(model_output, target)
            terms['loss'] = terms["train_dice"] + terms["mse_diff"] + 0.001*terms["mssim"]
        else:
            raise NotImplementedError(self.loss_type)
...
class FocalLoss(nn.Module):
    def __init__(self, weight=None, size_average=True, alpha_f=0.25, gamma_f=2.0):
