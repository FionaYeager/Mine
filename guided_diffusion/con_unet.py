...
class AT_ASPP(nn.Module):                       ##加入通道注意力机制
    def __init__(self, dim_in, dim_out, rate=1, bn_mom=0.1):
        ...
        self.branch1 = nn.Sequential(
            nn.Conv2d(dim_in, 64, 3, 1, padding=1, dilation=rate, bias=False),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
        ...
        self.branch2 = nn.Sequential(
            nn.Conv2d(dim_in, 64, 3, 1, padding=3 * rate, dilation=3 * rate, bias=False),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
        ...
        self.branch3 = nn.Sequential(
            nn.Conv2d(dim_in, 64, 3, 1, padding=5 * rate, dilation=5 * rate, bias=False),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
        ...
        self.branch5_conv = nn.Conv2d(dim_in, 64, 1, 1, padding=0, bias=False)
        self.branch5_bn = nn.BatchNorm2d(64)
        ...
        self.conv_cat = nn.Sequential(
            ...
            # nn.ReLU(inplace=True),
            nn.Conv2d(dim_out, dim_out, 1, 1, padding=0, bias=False),
            nn.BatchNorm2d(dim_out),
            nn.ReLU(inplace=True)
        )
        # print('dim_in:',dim_in)
...
class ConUNet(nn.Module):
    ...
    def __init__(self, n_channels, out_channels):
        ...
        self.ASPP = AT_ASPP(filters[3]+filters[2]+filters[1]+filters[0], filters[3])
        self.SE = SE_Block(filters[0]+filters[1]+filters[2]+filters[3])
    ...
    def forward(self, x):
        ...
        x_con = torch.cat((x1,x2,x3,e4),dim=1)
        x_con = self.SE(x_con)
        e4 = self.ASPP(x_con)
